# SOT – SMART MODERATOR V1.4.1  
**Sumber:** SOT V1.4 Final + Patchset V1.4.1 (Sesi 1–4 & Resolusi Gap Akhir, Diskusi Panel AI)  
...
**Backlog Panel:** Kimi, Qwen (belum aktif di V1.4.1 – ketidakhadiran tidak menggugurkan konsensus, tercatat di Friction Log)  
**Status:** **CLOSED – Siap Implementasi & Dry-Run (34 Item)**  
**Versi:** V1.4.1 (Final, Patchset V1.4 Sesi 1–5 + Patchset V1.4.1 Sesi 1–4 & Resolusi Gap Akhir)
...
20. Pending Micro-Decisions Before Dry-Run
...
| `Ref_ID` | Meta-LLM Filter | `Claim_ID` yang disanggah (jika `REBUTTAL`), `null` jika `NEW_IDEA`. **(V1.4.1)** Rujukan lintas topik (*cross-topic*) tetap diizinkan dalam satu `Session_ID`; klaim rebuttal-nya sendiri dicatat dengan `Topic_Session` topik aktif. |
...
| `Trigger_Source` | **(Revisi V1.4.1)** Wajib diisi di **semua** baris `VITAL_TRIGGER` (audit lengkap, tanpa kecuali). Enum sah (11 nilai): `CORE_PANEL_BUG_HUNT`, `SHADOW_SCRIBE`, `ZERO_CONTEXT_REVIEW`, `SCRIBE_VALIDATION`, `VITAL_GATE`, `API_ERROR`, `MAX_ROUND_LIMIT`, `WATCHDOG_DEADLOCK`, `MANUAL_OVERRIDE`, `STUCK_DETECTION`, `UNKNOWN`. `SHADOW_SCRIBE` = alert sah dari Scribe tentang isi debat; `SCRIBE_VALIDATION` = alert sistem tentang Scribe-nya sendiri yang salah/kurang (dipicu Tier 1 GAS atau Meta-LLM Cross-Check); `VITAL_GATE` = Vital Gate manual (§8.2, bukan hasil deteksi otomatis) — **menutup catatan terbuka V1.4**; `UNKNOWN` = fallback jika GAS gagal mengklasifikasi sumber, counter ≥3 dalam satu sesi → otomatis memicu `VITAL_TRIGGER` baru dengan `Trigger_Source=UNKNOWN` (mendeteksi degradasi parser, §8.12). **Larangan:** `SYSTEM_FRICTION` hanya boleh dipakai di kolom `Tipe`, dilarang masuk `Trigger_Source` (cegah *naming collision*). |
...
- **Watchdog 10 menit:** `WAITING_SCRIBE` >10 menit → force `SCRIBE_ERROR` + `Next_Action = BYPASS_LANJUT`. Jika berulang ≥2 kali dalam satu sesi topik → `VITAL_TRIGGER` (`Trigger_Source: WATCHDOG_DEADLOCK`).  
...
### 8.12 Pemisahan Jalur FATAL & `Trigger_Source` (Baru V1.4, Revisi V1.4.1)  
`VITAL_TRIGGER` (§7 Tab 4) **wajib** diisi `Trigger_Source` di **semua** baris, tidak hanya fatal — enum lengkap 11 nilai ada di §7 Tab 4. Tujuannya agar setiap pemicu (bug-hunt, scribe, 0-context review, vital gate manual, error, limit, dll.) bisa diaudit terpisah — jalur ini berbeda dari `Severity: FATAL` hasil `BUG_HUNT` (§8.14) yang masuk Vital Gate lewat rute sendiri.  
...
### 9.10 Disambiguasi Resume, Pause Manual, & Telegram Command (Baru V1.4.1)  
Mengatur perbedaan jeda manual Darma vs menunggu keputusan sistem, serta makna ganda perintah `/lanjut`.
...
- **`PAUSED_MANUAL` vs `WAITING_DARMA`:** `PAUSED_MANUAL` = Darma sengaja menjeda sistem (mis. via `/pause`). `WAITING_DARMA` = sistem menunggu keputusan vital/error. Dua kondisi berbeda, tidak boleh disatukan.  
- **Definisi pemicu `RESUME_CORE_PANEL`:** hanya dipicu ketika Darma mengirim `/lanjut` dari kondisi `Runtime_State = PAUSED_MANUAL`, **dan** tidak ada `VITAL_TRIGGER` terbuka — memastikan mesin tidak salah menyala saat masih menunggu keputusan vital.  
- **`RESUME_CORE_PANEL` vs `LANJUT` (Resolusi_Darma) — dua makna berbeda, wajib dibedakan:**  
  - `RESUME_CORE_PANEL` → melanjutkan mesin dari jeda manual (`PAUSED_MANUAL`).  
  - `LANJUT` (nilai `Resolusi_Darma`, §7 Tab 4) → skip AI yang error dalam rotasi normal (`WAITING_DARMA` akibat `SYSTEM_FRICTION`).  
- **Disambiguasi perintah `/lanjut` berdasarkan `Runtime_State` aktif saat diterima:**  
  - Jika `Runtime_State = PAUSED_MANUAL` + tidak ada `VITAL_TRIGGER` terbuka → `Next_Action = RESUME_CORE_PANEL`.  
  - Jika `Runtime_State = WAITING_DARMA` (error/friction) → `Resolusi_Darma = LANJUT` (skip AI), ikuti Flow §9.7 pattern.  
  - State lain → abaikan, log sebagai `UNKNOWN` command.  
- **(Revisi V1.4.1) Batas cakupan `LANJUT`:** Makna "skip AI" di atas baru **terkunci & teruji (Dry-Run Item 34) untuk kasus `Trigger_Source = API_ERROR`**. Untuk `WAITING_DARMA` dari `Trigger_Source` lain (`MAX_ROUND_LIMIT`, `STUCK_DETECTION`, `WATCHDOG_DEADLOCK`, dll.), arti operasional `LANJUT` **belum didefinisikan** — jangan diasumsikan otomatis berarti "skip AI". GAS wajib memperlakukan ini sebagai kasus belum tercover sampai didefinisikan eksplisit & diuji saat dry-run.
- **State setelah `RESUME_CORE_PANEL` dieksekusi:** GAS mengembalikan `Runtime_State = WAITING_AI` dan `Next_Action = NONE` — mencegah mesin terjebak di `PAUSED_MANUAL` setelah resume.
...
4. Isi `Round_Terakhir_Sanggah` = `Round_In_Topic` (untuk klaim baru) ATAU update baris `Ref_ID` jika `REBUTTAL`.  
...
| **KONTRAK BARU** | Semua baris `VITAL_TRIGGER` wajib `Trigger_Source` (audit lengkap, Opsi B), enum 11 nilai termasuk `VITAL_GATE`, `API_ERROR`, `MAX_ROUND_LIMIT`, `WATCHDOG_DEADLOCK`, `MANUAL_OVERRIDE`, `STUCK_DETECTION`, `UNKNOWN` (fallback, counter ≥3 → auto `VITAL_TRIGGER`). `SYSTEM_FRICTION` dikunci hanya untuk kolom `Tipe`. |
...
## 19. LANGKAH SELANJUTNYA (DRY-RUN) — REVISED CHECKLIST V1.4.1 (34 ITEM)

> §19 V1.3 (9 langkah, berbasis rotasi 5-AI dan struktur 5 tab) **diganti total** — sebagian besar item lama sudah tidak relevan dengan arsitektur Core Panel + Shadow Scribe + 8 tab. Item 22–34 ditambahkan di Patchset V1.4.1 untuk menguji `Trigger_Source`, `Next_Action`, `Topic_Session` lifecycle, struktur biaya, dan disambiguasi pause/resume.
...
> Section ini bukan bagian arsitektur final. Isinya adalah **keputusan taksonomi/perilaku yang masih terbuka** dan wajib dikunci sebelum build GAS dimulai. Setelah tiap poin diputuskan, pindahkan hasilnya ke section terkait & tandai sebagai RESOLVED.

### PMD-1: `Trigger_Source` untuk §8.8 Meta-LLM Filter Error & Observation Conflict

**Masalah:** SOT mengunci semua `VITAL_TRIGGER` wajib punya `Trigger_Source`. Tapi dua rute ini belum punya nilai yang pas:
- §8.8: Meta-LLM Filter gagal (output invalid JSON) → buat `VITAL_TRIGGER`, tapi nilai `Trigger_Source`-nya apa?
- `OBSERVATION_LEDGER`: observasi Darma bentrok `DECISION_LEDGER ACTIVE` → buat `VITAL_TRIGGER`, tapi nilai `Trigger_Source`-nya apa?

**Opsi yang perlu diputuskan:**

| Opsi | Nilai | Konsekuensi |
|---|---|---|
| A | Tambah `META_LLM_FILTER_ERROR` + `OBSERVATION_CONFLICT` (enum jadi 13) | Audit paling bersih, tapi mengubah kontrak V1.4.1 yang terkunci 11 nilai |
| B | Pakai `UNKNOWN` untuk keduanya, tapi `Pemicu` wajib sebut sumber eksplisit | Tidak ubah enum, audit kurang presisi |
| C | `META_LLM_FILTER_ERROR` saja +1 (jadi 12), Observation Conflict pakai `VITAL_GATE` | Kompromi — filter error dapat slot eksak, observation ikut gerbang vital |

→ **Darma perlu memutuskan pilihan A/B/C (atau kombinasi) sebelum build.**

---

### PMD-2: Guard Cross-Topic `Ref_ID` — Batas Perubahan Klaim Lama

**Masalah:** SOT sudah mengizinkan `Ref_ID` lintas `Topic_Session`, tapi belum ada aturan eksplisit:
- Apakah `Round_Terakhir_Sanggah` klaim lama ikut diupdate saat ada rebuttal dari topik baru?
- Apakah klaim lama yang sudah `STALE` bisa di-auto-`REJECTED` dari topik baru?

Tanpa aturan ini, implementasi GAS bisa merusak audit historis secara diam-diam.

**Rekomendasi panel (ChatGPT):** tambah guard minimal:
```
Cross-topic Ref_ID sah sebagai referensi, tapi tidak boleh otomatis mengubah Status
atau Round_Terakhir_Sanggah klaim lama. Perubahan klaim lintas topik hanya lewat
Decision Ledger / Vital Gate dengan keputusan eksplisit Darma.
```

→ **Darma perlu mengonfirmasi guard ini (terima/modifikasi/tolak) sebelum build.**

---

### PMD-3: Makna `LANJUT` dari `WAITING_DARMA` selain `API_ERROR`

**Masalah:** `LANJUT` dari `WAITING_DARMA` baru terkunci untuk `API_ERROR = skip AI`. Untuk sumber lain (`MAX_ROUND_LIMIT`, `STUCK_DETECTION`, `WATCHDOG_DEADLOCK`), makna operasionalnya belum didefinisikan — lihat §9.10.

**Opsi:**

| `Trigger_Source` | Makna `LANJUT` yang logis |
|---|---|
| `MAX_ROUND_LIMIT` | Tambah 1 round, atau konfirmasi ganti topik? |
| `STUCK_DETECTION` | Izinkan 1 round tambahan (dengan risiko tercatat)? |
| `WATCHDOG_DEADLOCK` | Bypass Scribe resmi (= `BYPASS_LANJUT`)? |
| `VITAL_GATE` | `LANJUT` tidak berlaku — wajib `A/B/REVISI/STOP` |

→ **Darma perlu memutuskan mapping per-sumber ini, atau mengunci "untuk semua sumber selain `API_ERROR`, `/lanjut` diabaikan sampai diuji dry-run" sebagai rule sementara.**

---

**=== STATUS AKHIR: CLOSED ===**

Panel AI telah mencapai **kesepakatan penuh** terhadap seluruh poin di atas, termasuk Patchset V1.4 (Sesi 1–5) dan Patchset V1.4.1 (Sesi 1–4 & Resolusi Gap Akhir).  
...
**Dokumen ini adalah Source of Truth (SoT) untuk Smart Moderator V1.4.1 Final — closed secara desain, siap implementasi dan Dry-Run §19 (34 item).**  
